...
public class Problem2 {
    ...
    public Problem2() {
        Arrays.sort(input);
        final int mean = getMean(input);
        System.out.println("mean: "+mean);
        boolean shouldGrow = true, shouldShrink = true;
        int currentBest = mean;
        int currentBestFuel = calculateFuel(input,currentBest);
        int currentOffset = 1;

//          Brute Force
//        for (int i = 0; i < input[input.length-1]; i++) {
//            final int newFuel = calculateFuel(input, i);
//            if(newFuel <= currentBestFuel) {
//                currentBestFuel = newFuel;
//                currentBest = i;
//            }
//        }

        while(shouldGrow || shouldShrink) {
            if(shouldGrow) {
                final int newFuel = calculateFuel(input, mean +currentOffset);
                if(newFuel <= currentBestFuel) {
                    currentBestFuel = newFuel;
                    currentBest = mean + currentOffset;
                } else {
                    shouldGrow = false;
                }
            }
        ...
        System.out.println("Current Best Value: "+currentBest);
        System.out.println("Current Best Fuel: "+currentBestFuel);
    }

    public int getMean(int[] arr) {
        final long sum = Arrays.stream(arr).asLongStream().sum();
        return (int) (sum/arr.length);
    }

    public int calculateFuel(int[] arr,int target) {
        System.out.println("Calculating fuel for "+target);
        int fuel = 0;
        for (int i = 0; i < arr.length; i++) {
            final int moving = Math.abs(target - arr[i]);
            final int summation = summation(moving);
//            System.out.println("moving "+moving + " from "+arr[i]+" to "+target);
//            System.out.println("summation "+summation);
    ...
    private int summation(final int target) {
        int val = 0;
    ...
    public int getClosestIndex(int[] arr,int target) {
        for (int i = 0; i < arr.length; i++) {
            if(arr[i] > target) {
                return i;
            }
